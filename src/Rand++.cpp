...
// Вызывается из CLI и из GUI-потока. progressFn — колбэк для прогресса.
...
static int RunCLI(int argc, wchar_t* argv[]) {
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (arg == L"-n" || arg == L"--count")  p.serie_count = _wcstoui64(getNext().c_str(), nullptr, 10);
        ...
        else if (arg == L"-p" || arg == L"--period") p.N = static_cast<size_t>(_wtoi(getNext().c_str()));
            ...
            std::wcout << L"Rand++ CLI mode (Cascade PRNG)\n"
                ...
                << L"  -p, --period N      Cascade steps N (default: 42)\n"
...
static INT_PTR CALLBACK RandDlgProc(HWND hDlg, UINT msg, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_COMMAND:
        ...
        case ID_SERIE_GENERATION_START: {
            ...
            p->N = static_cast<size_t>(_wtoi(buf));
            if (p->N < 1) p->N = 1;
