...
namespace perfect_random::crypto
{
	using namespace std;

	// ═══════════════════════════════════════════════════════════════
	//  SplitMix64 — независимое хеширование для инициализации (v2)
	// ═══════════════════════════════════════════════════════════════
	...
	//  v2: независимая инициализация + нелинейное смешивание выхода
	// ═══════════════════════════════════════════════════════════════
	...
	private:
		...
		// V2: нелинейное смешивание — скрывает внутреннее состояние от атакующего
		uint32_t current_value() const {
			uint64_t raw = (a * b + c) % M;
			// 4 раунда SplitMix64-подобного смешивания
			uint64_t z = raw;
			z = (z ^ (z >> 30)) * 0xBF58476D1CE4E5B9ULL;
			z = (z ^ (z >> 27)) * 0x94D049BB133111EBULL;
			z = z ^ (z >> 31);
	...
	public:
		...
		// V2: три независимых хеша через SplitMix64 — разрывает линейную связь a-b
		void seed(uint64_t s) {
			if (s == 0) s = 1;
			uint64_t sm_state = s;
			uint64_t h1 = splitmix64(sm_state);
			uint64_t h2 = splitmix64(sm_state);
			uint64_t h3 = splitmix64(sm_state);
